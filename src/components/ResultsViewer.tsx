  ...
  return (
    <div className="space-y-4">
      {results.map((result, index) => (
        <div key={index} className="border border-grok-secondary rounded-md overflow-hidden">
          <div className="bg-grok-secondary/20 p-3 flex justify-between items-center">
            <div className="flex items-center gap-2 text-sm">
              <span className="font-medium">Line {result.lineNumber}</span>
              {result.parsed ? (
                <Badge className="bg-grok-success">Matched</Badge>
              ) : (
          ...
          <div className="p-3 border-b border-grok-secondary bg-grok-darker">
            ...
            <div className="p-3 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2">
              {Object.entries(result.parsed).map(([key, value]) => (
                <div key={key} className="flex">
                  <div className="font-medium text-sm min-w-[120px] text-grok-primary">{key}:</div>
